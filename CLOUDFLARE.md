...
Local validation passes 79 Worker/D1 tests, eight browser-client tests, app and Worker/test typechecks, local migration application, Cloudflare web export and production dry run. Coverage includes real authenticated handler routing, direct approval, two-party swap, reject/decline/cancel, stale and simultaneous resolution, exact and changed retries, cross-team access, unrelated-member reason privacy, inactive shifts and membership revocation. Hosted migration, protected UI/API acceptance, CPU traces, final D1 state and rollback version remain pending.
