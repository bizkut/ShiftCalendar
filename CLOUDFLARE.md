...
Migration `0006_shift_change_requests.sql` is applied in production and Worker version `1b19b8cb-e57d-4502-81d7-68e38b68cac9` is live. Hosted acceptance used the existing two approved identities without adding Access seats. The member submitted a direct request from the assigned read-only team calendar for September 17, changing Night to Morning. The current team leader saw it in the manager queue, approved it, and confirmed both the approved history row and Morning roster after refresh. The request finished at version 2 and the roster day at version 2. D1 contains one create audit and one resolution audit, no pending requests, no transaction-check residue and no incomplete schedule runs. The private baseline rows remain Morning at versions 30 and 113. D1 was 389,120 bytes and served from APAC/HKG during the final query.

A live request-list trace used 3 ms CPU. The approval completed successfully under Workers Free, but terminal truncation discarded its exact CPU event; the evidence deliberately leaves that value null. Roll back code to version `43ab8a40-bc72-4fd1-92c7-2b60b780b42e` if needed and retain the forward-compatible request table. [Sanitized M6 evidence](cloudflare/live-evidence/2026-09-14-m6-change-requests.json).
