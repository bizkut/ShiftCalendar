...
The reusable policy permits only `bizkut.limau@gmail.com`. It protects all production traffic for this Worker; Worker previews remain disabled. No Bypass policy or account-wide protection was introduced. The app is restricted to the existing OTP provider, rather than accepting future identity providers automatically. The Worker independently verifies JWT signature, issuer, audience and expiry and reads active-user status from D1.
...
Workers remained Free after deployment (dashboard: Current plan); Zero Trust remained Free. Access had observed two historical pilot identities against the existing 50-user allowance, while the current allow policy contains one identity. D1 now has four databases, including the new dedicated ShiftCalendar database; the three unrelated databases and existing Workers/Pages/domains/tunnel were preserved.
...
## M8 — one-team rollout gate (in progress)

Worker version `8a123067-5a59-4579-bf80-69eac08cba9b` contains the M7 behavior plus an explicit **Check saved import result** action. The action reuses the same preview token, rows and mutation ID after a successful import so the manager can demonstrate the server's exact-retry result without writing the roster twice. Local validation passes 83 Worker/D1 tests, 11 browser-client tests, 14 pilot checks, all app/Worker/test typechecks, the 78-file Cloudflare web build, local migrations through `0007` and the production Wrangler dry run. A fresh Android local-only export passed; bundle scanning found no `shifts.amazonian.my`, Cloudflare Access cookie, cloud-session marker or cloud configuration variable.
...
Production has two active Access seats, one Access application, four of ten account-wide D1 databases, a 409,600-byte ShiftCalendar database and 48 retained Worker versions. Log sampling is 10%, trace sampling is 1%, invocation logs are disabled, production migrations are current and a Time Travel bookmark is available. The pre-cycle daily aggregate recorded 450 Worker requests, zero errors, 8,724 D1 rows read and 1,002 written. Worker CPU was P50 3.339 ms, P90 8.023 ms and P99 13.592 ms; P99 therefore remains a tail warning despite low shared-quota use. Final-version member route reads returned 200 with 1–6 ms CPU in the captured sample.

The rollback rehearsal sent 100% traffic to M7 version `e0dc8e5d-35bb-459a-b662-b8d12d59a46a` and verified Access protection for the application, API and asset. It then restored M8 candidate `641a0696-a20d-45e9-9a06-a561c1903439`. Because version restoration temporarily synchronized the general observability envelope at 100%, a normal production-config deployment created final version `8a123067-5a59-4579-bf80-69eac08cba9b` and reconfirmed the intended 10% log/1% trace settings. D1 was not rewound. The M7 recovery runbook and seven-day D1 Time Travel procedure remain current.

Do not onboard a second team yet. The open M8 gate is one grouped owner/member cycle on the documented September 23–24 fixtures: strict CSV and versioned JSON preview/apply, exact retry and conflict checks; member refresh and direct-change request; one manager resolution; export privacy; operation-specific CPU/D1 measurements; and final database invariants. If that passes, add at most one team, observe a normal seven-day window, and reassess before each further team. Stop on any privacy/authorization failure, incomplete or duplicate mutation, Worker 1102/repeated CPU termination, normal-cycle CPU P90 at or above 10 ms, 50% of a shared daily quota, or fewer than five unused Access seats. Free hosting remains conditional on Cloudflare's current plan and shared use; it is not a permanent cost guarantee. [Sanitized M8 checkpoint](cloudflare/live-evidence/2026-09-14-m8-rollout.json).
