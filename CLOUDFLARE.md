...
The reusable policy permits only the two approved pilot identities. It protects all production traffic for this Worker; Worker previews remain disabled. No Bypass policy or account-wide protection was introduced. The app is restricted to the existing OTP provider, rather than accepting future identity providers automatically. The Worker independently verifies JWT signature, issuer, audience and expiry and reads active-user status from D1.
...
Workers remained Free after deployment (dashboard: Current plan); Zero Trust remained Free. Access uses two approved pilot identities against the existing 50-user allowance. D1 now has four databases, including the dedicated ShiftCalendar database; the three unrelated databases and existing Workers/Pages/domains/tunnel were preserved.
...
## M4 completion — shared team roster

Version `84062431-643f-4014-9ea8-9d2ddbf772e7` exposes `GET /v1/teams/:teamId/roster` and `PATCH /v1/teams/:teamId/roster/:memberSub/days/:date`. The read endpoint is limited to 31 inclusive calendar dates and 100 explicitly selected rows per cursor page. It requires a current active team membership and excludes private notes, overtime, leave, pay and swap details. The write endpoint resolves the active team calendar for the assigned member, then rechecks the active leader/manager role, active assigned membership, day revision and retry identity in the atomic D1 batch. Application administrator status does not replace the team role.

The browser groups calendars under **My shifts** and **Team roster · team name**, with one member calendar chip per assigned roster. Team calendars hide personal-detail, template, repeat and note-search controls. Leaders/managers can edit a single date. Members/viewers see a read-only notice and no mutation controls. Cloud team bulk scheduling returns a truthful M5 message instead of acknowledging an unsaved change.

Local validation passed 69 Worker/D1 tests and six browser-client tests, all typechecks, Expo web and Android exports, and a production dry run. Hosted acceptance reused the active first-user session, then made one final switch to the second user with one PIN. The leader wrote Night (23:00–07:00) on 17 September 2026; after refresh both leader and member UIs showed it. The member's roster PATCH returned 403, the read-only UI exposed no edit/private controls, and the roster JSON contained no private fields. Live roster reads used 1, 1 and 3 ms CPU; the denied roster write path used 5 ms. The successful leader write was confirmed by the member and its audit/D1 row; its individual tail event was not captured. All captured M4 requests stayed below the 10 ms Free-plan gate.

Final D1 checks show the first user active as the sole application administrator and team owner, the second active as a standard member, no pending invitations, zero transaction-check residue, unchanged private Morning rows at versions 30 and 113, the existing Afternoon roster row at version 3 and the new Night row at version 1. No migration or paid service was added. Roll back the Worker to M3 version `841deab6-458a-4735-b4cc-6826318980b5`; leave D1 intact. Full sanitized results are in [the M4 evidence record](cloudflare/live-evidence/2026-09-14-m4-roster.json).

## M5 — scheduling tools (complete)

Migration `0005_team_scheduling.sql` adds `team_shift_types`, `team_rotation_templates` and `schedule_runs` without changing existing calendar, membership or private-data rows. Custom shift codes are stable; records carry labels, colors, icons, start/end times, versions and active/archived state. Archiving blocks new assignments while historical roster rows continue to render. Rotation templates store an ordered shift-code cycle with revisions and archive state.

Current team leaders/managers can manage these records and preview explicit member/date assignments. The API validates the team, active members, assigned team calendars, active shift codes, template/day revisions and intact preview before apply. Application administrator status alone grants no scheduling permission. Members/viewers remain read-only, and shared responses exclude notes, overtime, leave, pay and swap data.

Worker version `43ab8a40-bc72-4fd1-92c7-2b60b780b42e` permits two assignments per preview, enough for two dates for one member or one date across two members. The apply path combines authorization/template/calendar reads, preclassifies known stale entries, and commits current day writes with the final run result and audits in one atomic D1 batch. A race that invalidates the batch falls back to guarded per-entry writes. The browser retains one mutation ID for retry, displays every applied/conflict outcome, offers a saved-result check, and refreshes only when the leader taps Done.

Hosted acceptance created the overnight `L` shift (`Late`, 22:00–06:00) and the `M5 Late / Rest` cycle (`L/O`). The restored member saw the documented roster and had no scheduling mutation controls. On the final version, a representative preview used **4 ms CPU**, a fresh two-success apply used **6 ms**, a one-success/one-conflict stale apply used **4 ms**, and its exact retry used **4 ms**. The retry returned the stored result and D1 retained exactly one schedule audit and one day audit for the successful entry. Earlier 12, 16, 11 and 18 ms candidate samples are retained as failed performance evidence; those findings drove the two-entry bound and atomic batching.

Local validation passes 73 Worker/D1 tests and seven browser-client tests, all app/Worker/test typechecks, the 78-asset Cloudflare web build, Android local-only export and production dry run. Production ends with two active users, one active application administrator, owner/member team roles, no pending invitations, no transaction-check residue and no incomplete schedule runs. Both private Morning rows remain unchanged at versions 30 and 113. D1 is 356,352 bytes after the acceptance fixtures. The first user remains active as application administrator and team leader. Roll back code to version `2d494c0b-eb79-420b-8b10-a9b348821c28` if needed and leave D1 intact. [Sanitized M5 evidence](cloudflare/live-evidence/2026-09-14-m5-scheduling.json).

## M6 — shift change requests

Migration `0006_shift_change_requests.sql` adds an indexed request table with direct/swap kind, requester and optional counterpart targets, observed day versions/shift codes, a private reason, explicit `pending_counterpart`/`pending_manager` and final states, request version, timestamps and resolver identity. It is additive and leaves existing calendars, roster days and M5 scheduling records unchanged.

Members and viewers can submit only from their own assigned team calendar. Direct changes wait for a current leader/manager. A swap names another current team member and assigned calendar/date, records both observed shifts, and requires counterpart acceptance before manager review. Approval rechecks current active users, memberships, assigned calendars, request state/revision, roster revisions and active shift codes in one D1 batch; it updates one direct date or exchanges both swap dates, closes the request and records mutation/audit rows atomically. Rejection, decline and cancellation never write roster days. Exact retries are idempotent, changed mutation payloads conflict, and demotion/removal prevents replay because current authorization precedes stored-result return.

The calendar-day sheet offers requests only on the signed-in member's assigned shift. The form supports a desired shift or counterpart/date selection and loads the counterpart's current roster entry before submission. The Teams screen lists only relevant history or the current manager queue, opens private reasons separately, and shows accept/decline/cancel/approve/reject actions by role and state. Client mutation IDs persist across failed retries and are cleared only after a confirmed response; roster refresh follows confirmed approval.

Local validation passes 80 Worker/D1 tests, eight browser-client tests, app and Worker/test typechecks, local migration application, Cloudflare web export and production dry run. Coverage includes real authenticated handler routing, direct approval, two-party swap, reject/decline/cancel, stale and simultaneous resolution, exact and changed retries, overlapping-request rejection, input/page bounds, cross-team access, unrelated-member reason privacy, inactive shifts and membership revocation.

Migration `0006_shift_change_requests.sql` is applied in production and Worker version `1b19b8cb-e57d-4502-81d7-68e38b68cac9` is live. Hosted acceptance used the existing two approved identities without adding Access seats. The member submitted a direct request from the assigned read-only team calendar for September 17, changing Night to Morning. The current team leader saw it in the manager queue, approved it, and confirmed both the approved history row and Morning roster after refresh. The request finished at version 2 and the roster day at version 2.

A second hosted run covered a real two-party swap. The member submitted September 18 Late against the leader's documented September 21 Morning fixture. The leader accepted as the selected counterpart and then approved as manager. After refresh, the member date was Morning, the leader date was Late, both rows were version 2, and the request was approved at version 3. Live CPU was 10 ms for submit, 3 ms for list, 5 ms for counterpart response and 7 ms for manager resolution. D1 contains two request-create audits, one response audit and two resolution audits, with no pending request, unresolved mutation, transaction-check residue or incomplete schedule run. The private baseline rows remain Morning at versions 30 and 113. D1 was 397,312 bytes and served from APAC/HKG during the final query. Roll back code to version `43ab8a40-bc72-4fd1-92c7-2b60b780b42e` if needed and retain the forward-compatible request table and documented M6 roster fixtures. [Sanitized M6 evidence](cloudflare/live-evidence/2026-09-14-m6-change-requests.json).

## M7 — migration, export and recovery

Worker version `e0dc8e5d-35bb-459a-b662-b8d12d59a46a` and migration `0007_team_import_runs.sql` add manager-only, two-row CSV/JSON roster import with preview, mapping validation, conflict reporting and exact retry idempotency. The same panel downloads paginated roster, request and audit data as JSON or spreadsheet-safe CSV. Request reasons and private calendar fields never enter these exports. Hosted leader apply and downloads passed on candidate `52e09446-df05-4a89-91cc-dab4658089b9`; the member saw the imported shared shift but had no migration controls and retained the read-only team day sheet. The final version adds strict CSV column-count rejection. Live candidate reads measured 3–10 ms CPU.

The data drill used disposable APAC databases and sanitized fixtures only. Time Travel restored Morning/version 1 after a Night/version 2 mutation. An independent D1 SQL export/import reproduced users, a team, memberships, a roster day, an approved request, a mutation, an audit event and a completed import record. Temporary databases and the local SQL file were deleted. Take application exports before structural imports and weekly during the pilot, keep them encrypted for 30 days, and rely on D1 Free Time Travel only for its separate rolling seven-day window. Follow [the recovery runbook](cloudflare/RECOVERY.md); never rehearse a restore against production.

The code drill sent 100% traffic to retained version `1b19b8cb-e57d-4502-81d7-68e38b68cac9`; the custom domain and Access-protected app, API and asset routes remained reachable. Candidate `52e09446-df05-4a89-91cc-dab4658089b9` was then restored to 100%, followed by final version `e0dc8e5d-35bb-459a-b662-b8d12d59a46a`. Code rollback retains the additive table and does not rewind data. [Sanitized M7 evidence](cloudflare/live-evidence/2026-09-14-m7-migration-recovery.json).

## M8 — one-team rollout gate (complete)

Worker version `ea4a78a5-0157-4326-af76-5de7ac9843c4` contains the M7 behavior plus an explicit **Check saved import result** action and a separate **Done** action. The panel remains open after apply so a manager can demonstrate the server's exact-retry result without writing the roster twice, then refresh the roster deliberately. Local validation passes 83 Worker/D1 tests, 11 browser-client tests, 14 pilot checks, all app/Worker/test typechecks, the 78-file Cloudflare web build, local migrations through `0007` and the production Wrangler dry run. A fresh final-source Android local-only export passed; bundle scanning found no `shifts.amazonian.my`, Cloudflare Access cookie, cloud-session marker or cloud configuration variable.

The final-version member session passed direct navigation and reload of `/teams`, retained Saved to cloud state, showed the assigned calendar as read-only and exposed no scheduling, import or export controls. A 492×760 responsive browser viewport had no horizontal overflow; this is browser layout evidence, not a physical-device test. Direct member calls to import preview, roster export and the leader's private calendar returned 403. The focused security sample passed 46 tests covering Access JWT claims/signatures and forgery, exact origin/CSRF behavior, private isolation, current-role checks before retry, stale/concurrent requests, inactive/removed users, exact import retry, changed/stale import conflicts and export omission of request reasons. Public app, nested route, API and asset checks redirect to Access, while the disabled `workers.dev` hostname returns 404.

Production has two active Access seats, one Access application, four of ten account-wide D1 databases, a 417,792-byte ShiftCalendar database and 49 retained Worker versions. Log sampling is 10%, trace sampling is 1%, invocation logs are disabled, production migrations are current and a Time Travel bookmark is available. The pre-cycle daily aggregate recorded 450 Worker requests, zero errors, 8,724 D1 rows read and 1,002 written. Worker CPU was P50 3.339 ms, P90 8.023 ms and P99 13.592 ms; P99 therefore remains a tail warning despite low shared-quota use. On the final version, the captured exact import retry and exact request-resolution retry each used 9 ms CPU. Captured GET routes used 0–4 ms CPU and all sampled outcomes were successful.

The rollback rehearsal sent 100% traffic to M7 version `e0dc8e5d-35bb-459a-b662-b8d12d59a46a` and verified Access protection for the application, API and asset. It then restored M8 candidate `641a0696-a20d-45e9-9a06-a561c1903439`. Because version restoration temporarily synchronized the general observability envelope at 100%, normal production-config deployments restored the intended 10% log/1% trace settings and produced final version `ea4a78a5-0157-4326-af76-5de7ac9843c4`. D1 was not rewound. The M7 recovery runbook and seven-day D1 Time Travel procedure remain current.

The grouped cycle passed. The leader previewed and applied strict CSV and versioned JSON schedules; exact retries returned their stored results, a changed payload returned 409, and a stale preview reported one conflict without replacing the concurrent edit. The member refreshed to the imported September 23 Afternoon shift, requested Night from the read-only calendar, and saw the approved request and Night shift after one leader resolution. Roster, request and audit exports excluded request reasons and private calendar fields. Final D1 state was September 23 Night/version 3 and September 24 Afternoon/version 2, with the request approved at version 2, five completed import runs, and no pending request, incomplete import/schedule run or transaction-check residue. The established private baseline rows remained unchanged.

M8 accepts the current one-team production deployment and makes a deliberate no-go decision for immediate expansion. Observe it for seven stable days, then add at most one team and reassess before each further team. Stop on any privacy/authorization failure, incomplete or duplicate mutation, Worker 1102/repeated CPU termination, normal-cycle CPU P90 at or above 10 ms, 50% of a shared daily quota, or fewer than five unused Access seats. Free hosting remains conditional on Cloudflare's current plan and shared use; it is not a permanent cost guarantee. [Sanitized M8 evidence](cloudflare/live-evidence/2026-09-14-m8-rollout.json).
