...
## M5 candidate — scheduling tools

Migration `0005_team_scheduling.sql` adds `team_shift_types`, `team_rotation_templates` and `schedule_runs` without changing existing calendar, membership or private-data rows. Custom shift codes are stable; records carry labels, colors, icons, start/end times, overnight behavior through their times, versions and active/archived state. Archiving removes a shift from new assignments while historical roster rows continue to render. Rotation templates store an ordered shift-code cycle with revisions and archive state.

Current team leaders/managers can manage these records and preview explicit member/date assignments. The API validates the team, active members, assigned team calendars, active shift codes, template/day revisions and the intact preview token before apply. Each child assignment uses its own stable mutation ID and atomic current-user, current-role, target-membership, active-shift and revision guards. The enclosing schedule run stores the request fingerprint and final per-entry results, so an exact retry returns the same result and a changed-payload retry conflicts. Members/viewers and application administrators without a qualifying team role cannot apply schedules. Shared responses continue to exclude notes, overtime, leave, pay and swap data.

The first deployed M5 candidate allowed 14 assignments. Hosted UI acceptance created the overnight `L` shift (`Late`, 22:00–06:00), saved the `M5 Late / Rest` cycle (`L/O`) and applied `L/O/L` to the second member for 18–20 September 2026. A separate browser tab changed 31 December after a two-day preview; applying that stale preview produced one applied row and one conflict with “This day changed. Refresh before retrying.” D1 stored both truthful outcomes. After restoring the second identity to member version 5, refresh retained `L/O/L`, removed the Schedule control and displayed the read-only team notice.

That stale apply returned HTTP 200 at **12 ms CPU** on version `2ac69103-787d-4e5e-a2b9-25491bcc4e71`. It is retained as a failed Free CPU-gate sample. Query accounting also showed 14 entries could exceed the current Workers Free limit of 50 D1 queries per invocation. Source commit `4015dba` therefore caps preview/apply at four assignments, reads all current day revisions in one query, reuses preview-resolved calendar IDs and retains the transaction-boundary checks. Including the handler's active-user query, the worst all-conflict path is bounded at 50 D1 queries; the four-success path uses fewer. Version `d0ff73c7-8c88-4468-84d4-c40aecd7d981` is deployed with this correction. It still needs live preview, maximum-size apply, partial-conflict and exact-retry CPU measurements before M5 completion. Roll back to `2ac69103-787d-4e5e-a2b9-25491bcc4e71` only for code comparison; that version has the unsafe larger bound.

Local validation passes 73 Worker/D1 tests and seven browser-client tests, all app/Worker/test typechecks, the Cloudflare web build, Android local-only export and production dry run. Coverage includes overnight/archive history, cross-year previews, two-member four-entry apply, bounds, preview/apply parity, stale partial results, exact and changed retries, team boundaries, demotion/removal/deactivation, archived shifts and private-field isolation. The hosted database remains 303,104 bytes with two active users, one active application administrator, owner/member team roles, no pending invitations and no transaction-check residue. Both private Morning rows remain unchanged at versions 30 and 113. [Sanitized M5 evidence](cloudflare/live-evidence/2026-09-14-m5-scheduling.json).
