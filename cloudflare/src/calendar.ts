...
export class CalendarRepository {
  ...
  async prepareScheduledRosterDay(teamId: string, item: { memberSub: string; calendarId: string; date: string;
  ...
  async writeScheduledRosterDays(teamId: string, assignments: Array<{ memberSub: string; calendarId: string; date: string;
    ...
    const prepared = await Promise.all(assignments.map(item => this.prepareScheduledRosterDay(teamId, item)));
    await this.db.batch(prepared.flatMap(item => item.statements));
