## Email and login-code checks

- Use the `codex_apps` Gmail MCP connector to check email when a task requires a login PIN or verification message.
- Before searching for a code, verify the connector profile matches the message's recipient. If the connected inbox is different, state that limitation and do not claim to have checked the intended inbox.
- Reuse active authenticated sessions and avoid requesting replacement login codes unless the current code has expired or is unavailable.
